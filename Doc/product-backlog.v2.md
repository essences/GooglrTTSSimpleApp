...
更新日: 2025-11-25 (Epic B 主要機能実装完了)
...
**今後の優先タスク**:
1. **B4**: 複数セクション分割生成（原稿を章・段落ごとに自動分割し、個別生成・ダウンロード）
2. **B6**: エラーハンドリング強化（リトライロジック、詳細なエラーメッセージ）
3. **Playwright テスト更新**: 新機能（試聴、TXT インポート、履歴永続化）のテストケース追加
4. **パフォーマンス最適化**: 大量履歴時のテーブル描画最適化、仮想スクロール検討
